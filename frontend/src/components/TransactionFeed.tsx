...
function LSTMBar({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-1.5 mt-1">
      ...
      <span className="text-xs text-slate-500 w-7 text-right font-mono">{pct}%</span>
    </div>
  );
}
...
function TransactionRow({ tx }: { tx: Transaction }) {
  ...
  return (
    <div className={`px-3 py-2.5 border-b transition-colors ${
      tx.isActive
        ? isHighAlert
      ...
      <div className="flex items-start justify-between gap-2">
        {/* Left: channel icon + merchant */}
        <div className="flex items-center gap-2 min-w-0">
          <div className={`p-1 rounded shrink-0 ${tx.isActive ? 'text-amber-400' : 'text-slate-500'}`}>
      ...
      {/* Active investigation label */}
      {tx.isActive && (
        <p className="text-xs text-amber-500/80 font-medium mt-1 truncate">
...
export default function TransactionFeed({ transactions }: Props) {
  return (
    <aside className="w-64 shrink-0 bg-slate-800 border border-slate-700 rounded-lg flex flex-col overflow-hidden">
      {/* Header */}
      <div className="px-3 py-2.5 border-b border-slate-700 shrink-0">
        <p className="text-xs font-semibold text-slate-300 uppercase tracking-wider">
          Live Transaction Feed
        </p>
        <p className="text-xs text-slate-500 mt-0.5">Real-time · 184,299 today</p>
      </div>
